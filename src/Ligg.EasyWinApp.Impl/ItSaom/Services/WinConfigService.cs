...
namespace Ligg.EasyWinApp.Implementation.Services
{
    internal class WinConfigService
    {
        ...
        public void RefreshSelectedWinConfigGroups()
        {
            try
            {
                foreach (var id in WinConfigServiceData.SelectedWinConfigGroupIds)
                {
        ...
        public void AutoFixSelectedWinConfigGroups()
        {
            try
            {
                foreach (var id in WinConfigServiceData.SelectedWinConfigGroupIds)
        ...
        public void SaveSelectedWinConfigGroups()
        {
            try
            {
                foreach (var id in WinConfigServiceData.SelectedWinConfigGroupIds)
        ...
        public void AddOrRemoveSelectedWinConfigGroupId(int id, bool Add)
        {
            try
            {
                if (Add)
                {
                    if (!WinConfigServiceData.SelectedWinConfigGroupIds.Contains(id))
                    {
                        WinConfigServiceData.SelectedWinConfigGroupIds.Add(id);
                    }
                }
                else
                {
                    if (WinConfigServiceData.SelectedWinConfigGroupIds.Contains(id))
                    {
                        WinConfigServiceData.SelectedWinConfigGroupIds.Remove(id);
                    }
                }
            }
            catch (Exception ex)
            {
                throw new ArgumentException("\n>> " + GetType().FullName + ".AddOrRemoveSelectedWinConfigGroupId Error: " + ex.Message);
            }
        }

        public string GetSelectedWinConfigGroupIds()
        {
            try
            {
                if (WinConfigServiceData.SelectedWinConfigGroupIds.Count > 0)
                {
                    return StringExtension.ConvertIntegerListToIdsString<int>(WinConfigServiceData.SelectedWinConfigGroupIds, ',');
                }
            ...
            {
                throw new ArgumentException("\n>> " + GetType().FullName + ".GetSelectedWinConfigGroupIds Error: " + ex.Message);
            }
        }
    ...
    internal static class WinConfigServiceData
    {
        ...
        public static List<int> SelectedWinConfigGroupIds = new List<int>();
